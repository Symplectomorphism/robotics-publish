...
## Angle-axis representation {.smaller}
...
* Derive a rotation matrix $\bm{R}_{k,\theta}$ representing a rotation
    of $\theta$ about axis $\bm{k}$.
  - The rotational transformation $\bm{R} = \bm{R}_{z,\alpha}\bm{R}_{y,\beta}$ 
    brings the world $z$-axis into alignmend with the vector $\bm{k}$.
...
k_x^2v_\theta + c_\theta & k_xk_yv_\theta - k_zs_\theta & k_xk_yv_\theta + k_ys_\theta \\
k_xk_yv_\theta + k_zs_\theta & k_y^2v_\theta +c_\theta & k_yk_zv_\theta - k_xs_\theta \\
k_xk_yv_\theta - k_ys_\theta & k_yk_zv_\theta + k_xs_\theta & k_z^2v_\theta + c_\theta
\end{pmatrix}
$$
:::
...
::: {.column width="55%"}
* Angle-axis representation characterizes a given rotation by four quantities:
...
::: {.callout-tip icon="false"}
## CAUTION
Two angle-axis representations may **NOT** be combined using standard rules of
...
$$
\bm{R} = \bm{R}_{x,60^\circ}\bm{R}_{y,30^\circ}\bm{R}_{z,30^\circ} = 
\begin{pmatrix}
