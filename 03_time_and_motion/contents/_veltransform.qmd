...
another is referred to as the _adjoint transformation_ associated with $\bm{T}$.
  - In the above calculation $\text{Ad}_\bm{T}$ maps body velocity to spatial
    velocity.
  - $\text{Ad}_\bm{T}$ is invertibe and its inverse is given by
  $$
...
\end{aligned}
$$
* Using this ${^A}\bm{\omega}$, we compute
$$
...
$$
This is precisly the velocity of a point attached to the rigid body as it
...
## Coordinate transformations {.smaller}
