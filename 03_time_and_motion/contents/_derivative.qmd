...
the two matrices (first equality) which comprise $\bm{A}$ have mutually orthogonal nullspaces 
when $\theta \neq 0$ (and $\bm{R} \neq \bm{I}$).
:::
...
us view each side of this equation as a linear transformation. 

The right-hand side transformation acts on a vector $\bm{v} \in \mathbb{R}^3$ to produce its
projection along $\bm{\omega}$: $\bm{\omega}(\bm{\omega}^\top \bm{v}) =
(\bm{\omega}^\top \bm{v})\bm{\omega}$.  The component of $\bm{v}$ orthogonal to
$\bm{\omega}$ is mapped to zero: $\bm{\omega\omega}^\top\left(\bm{v} -
(\bm{\omega}^\top \bm{v})\bm{\omega}\right) = \bm{0}$.

Indeed, the left-hand side transformation acts on any vector in exactly the same 
manner. If $\bm{v} \parallel \bm{\omega}$ then $(\bm{I} + \hat{\bm{\omega}}^2)\bm{v} =
\bm{v} + \bm{\omega} \times (\bm{\omega} \times \bm{v}) = \bm{v}$. If, on the 
other hand, $\bm{v} \perp \bm{\omega}$ then $\bm{\omega} \times (\bm{\omega} 
\times \bm{v}) = -\bm{v}$. Hence $(\bm{I} + \hat{\bm{\omega}}^2)\bm{v} = \bm{0}$.

This shows that, as linear transformations, $\bm{I} + \hat{\bm{\omega}}^2$ and 
$\bm{\omega}\bm{\omega}^\top$ act on any vector in exactly the same manner. 
Therefore they are the same transformation.
:::
