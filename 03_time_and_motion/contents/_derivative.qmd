...
Let $\bm{s} \in S$, so that $\bm{s} = \alpha \bm{\omega}$ for some $\alpha \in \mathbb{R}$. We have 
$\bm{\omega}\bm{\omega}^\top \bm{s} = \bm{s}$. On the other hand, $(\bm{I} + \hat{\bm{\omega}}^2)\bm{s} = \bm{s} + \bm{\hat{\omega}}(\bm{\omega} \times \bm{s}) = \bm{s}$.
Hence $\bm{\omega}\bm{\omega}^\top = \bm{I} + \hat{\bm{\omega}}^2 = \bm{I}$ on $S$.

Let $\bm{n} \in N$. Then we have 
$\bm{\omega}\bm{\omega}^\top \bm{n} = \bm{0}$. On the other hand, $(\bm{I} + \hat{\bm{\omega}}^2)\bm{n} = \bm{n} + \bm{\hat{\omega}}(\bm{\omega} \times \bm{n}) = \bm{0}$.
